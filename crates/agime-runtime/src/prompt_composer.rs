//! Agent prompt composer (runtime side).
//!
//! Migrated from `agime-team-server::agent::agent_prompt_composer`. Only the
//! import path of `RuntimeCapabilitySnapshot` changed — it now lives in
//! [`crate::capability_types`] alongside the supporting capability data
//! shapes. Behaviour is identical and team-server still re-exports the same
//! public symbols via a thin shim.
...
use crate::capability_types::RuntimeCapabilitySnapshot;
    ...
    fn sample_snapshot() -> RuntimeCapabilitySnapshot {
        RuntimeCapabilitySnapshot {
            ...
            extensions: RuntimeExtensionResolution {
                builtin_capabilities: vec![ConfiguredBuiltinCapability {
