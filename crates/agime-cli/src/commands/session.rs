...
use agime::session::{generate_diagnostics, Session, SessionManager};
...
fn export_session_to_markdown(
    messages: Vec<agime::conversation::message::Message>,
    session_name: &String,
) -> String {
    ...
    for message in &messages {
        ...
        let is_only_tool_response = message.role == rmcp::model::Role::User
            && message.content.iter().all(|content| {
                matches!(
                    content,
                    agime::conversation::message::MessageContent::ToolResponse(_)
                )
            });
        ...
        if message.content.iter().any(|content| {
            matches!(
                content,
                agime::conversation::message::MessageContent::ToolRequest(_)
            )
        }) {
