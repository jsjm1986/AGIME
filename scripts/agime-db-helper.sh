...
BACKUP_DIR="${HOME}/.local/share/agime/agime-db-backups"
...
MIGRATIONS_DIR="${HOME}/.local/share/agime/migrations"
...
get_agime_db_path() {
    if [[ -n "${AGIME_PATH_ROOT:-}" ]]; then
        echo "${AGIME_PATH_ROOT}/data/sessions/sessions.db"
    else
        local possible_paths=(
            "${HOME}/.local/share/agime/sessions/sessions.db"
            "${HOME}/Library/Application Support/Block/agime/data/sessions/sessions.db"
        )
...
show_status() {
    echo -e "${BLUE}=== AGIME Database Status ===${NC}"
    ...
    if [[ ! -f "${DB_PATH}" ]]; then
        ...
        echo ""
        echo "This is normal if you haven't run AGIME yet."
        echo "Once you run AGIME, a database will be created automatically."
        return
    fi
...
generate_migrations() {
    if [[ ! -f "${RUST_SESSION_MANAGER}" ]]; then
        ...
        echo "Make sure you're running this from the AGIME repository root."
        exit 1
    fi
...
show_help() {
    ...
    echo -e "${BLUE}AGIME Database Migration Helper${NC}"
    echo ""
    ...
    echo "    rm -rf ~/.local/share/agime/migrations/004_*"
    echo "    $0 generate-migrations"
    echo ""
