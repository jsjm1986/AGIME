...
4. Include explicit completion evidence only when it is necessary to validate the final result or explain a real blocker.
5. For engineering work, prefer a lightweight quality pass over unverified "it should work" claims, but do not turn logs or evidence scraps into mission-level deliverables unless the user asked for them.
6. The earliest step should materially create or advance a requested asset. Do not start with pure orientation, workspace confirmation, or narration.
7. Required artifacts and completion checks should name end-user deliverables by default, not planning notes, bookkeeping files, logs, or generic evidence scraps.
...
- For research or comparison work, the first step should usually create a reusable result artifact such as a comparison table, structured notes, or a draft report section instead of a generic exploration step.
...
- For runtime-facing goals, include a local quality/verification step before deployment. Only add a blocked/handoff artifact if the environment truly prevents completion.
