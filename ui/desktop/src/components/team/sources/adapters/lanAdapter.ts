// LAN Data Source Adapter
// Handles communication with LAN peer devices
...
export class LANAdapter implements DataSourceAdapter {
  private source: DataSource;

  constructor(source: DataSource) {
    if (source.type !== 'lan') {
      throw new Error('LANAdapter requires a LAN data source');
    }
    this.source = source;
  }
  ...
  async isAvailable(): Promise<boolean> {
    try {
      const response = await fetch(`${this.getBaseUrl()}/health`, {
        method: 'GET',
        signal: AbortSignal.timeout(3000), // Shorter timeout for LAN
      });
...
/** Create a LAN adapter from source configuration */
export function createLANAdapter(source: DataSource): LANAdapter {
  return new LANAdapter(source);
}
