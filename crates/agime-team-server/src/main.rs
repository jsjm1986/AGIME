...
enum Commands {
    ...
    /// Audit digital avatar deep-water risks without mutating runtime data
    AuditAvatarDeepWater {
        /// Limit audit to a specific team id
        #[arg(long)]
        team_id: Option<String>,
    },
}
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Some(Commands::AuditAvatarDeepWater { team_id }) => {
            run_avatar_deep_water_audit(team_id).await
        }
...
async fn run_avatar_deep_water_audit(team_id: Option<String>) -> Result<()> {
    dotenvy::dotenv().ok();
    let _ = tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
    ...
    let config = Config::from_env()?;
    if !matches!(config.database_type, DatabaseType::MongoDB) {
        anyhow::bail!("avatar deep-water audit only supports MongoDB backend");
    }

    let mongo = MongoDb::connect(&config.database_url, &config.database_name).await?;
    let svc = agent::service_mongo::AgentService::new(Arc::new(mongo));
    let report = svc.audit_avatar_deep_water(team_id.as_deref()).await?;
...
async fn run_server(port_override: Option<u16>) -> Result<()> {
    ...
    info!(
        "DirectHarness V4 surfaces active: chat, channel, document, scheduled-task, AgentTask, subagent/swarm; no legacy task bridge"
    );
...
fn build_router(state: Arc<AppState>) -> Router {
    ...
    let public_routes = match &state.db {
        DatabaseBackend::MongoDB(_) => Router::new()
            ...
            .route(
                ...
                post(auth::routes_mongo::register_from_invite),
            )
    ...
    if let DatabaseBackend::MongoDB(db) = &state.db {
        ...
        tokio::spawn(async move {
            let orchestrator =
                agent::chat_channel_orchestrator::ChatChannelOrchestratorService::new(
                    db.clone(),
    ...
    let automation_routes = match (&state.db, &chat_manager) {
        (DatabaseBackend::MongoDB(db), Some(cm)) => Some(
            automation::router(db.clone(), cm.clone(), state.config.workspace_root.clone()).layer(
                axum::middleware::from_fn_with_state(
                    state.clone(),
                    auth::middleware_mongo::auth_middleware,
                ),
