...
/**
 * Script to unregister ALL agime:// and goose:// protocol handlers
 * Usage: node scripts/unregister-deeplink-protocols.js
 */

const { execSync } = require('child_process');

const PROTOCOL = 'agime';
const LEGACY_PROTOCOL = 'goose';

function unregisterAllProtocolHandlers() {
  console.log('Unregistering ALL agime:// and goose:// protocol handlers...');

  try {
    // Get all registered AGIME and Goose apps
    console.log('Finding all registered AGIME and Goose applications...');
    const lsregisterOutput = execSync(`/System/Library/Frameworks/CoreServices.framework/Versions/A/Frameworks/LaunchServices.framework/Versions/A/Support/lsregister -dump | grep -B 10 -A 10 -E "claimed schemes:.*(${PROTOCOL}|${LEGACY_PROTOCOL}):"`, { encoding: 'utf8' });
    ...
    if (pathMatches) {
      pathMatches.forEach(match => {
        ...
        if (path.includes('AGIME') || path.includes('agime') || path.includes('Goose') || path.includes('goose')) {
          uniquePaths.add(path);
        }
      });
    }

    console.log(`Found ${uniquePaths.size} app(s) to unregister:`);
    ...
    console.log(`\n✅ Successfully processed ${unregisteredCount} applications`);
    console.log('All agime:// and goose:// protocol handlers have been unregistered.');
    ...
    console.log('1. Use Activity Monitor to quit all AGIME/Goose processes');
    console.log('2. Delete AGIME/Goose apps from Applications folder');
