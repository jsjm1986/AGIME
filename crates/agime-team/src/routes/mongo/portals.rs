...
async fn read_portal_preview_bytes(
    ...
    let is_index = relative_path.is_empty() || relative_path == "index" || relative_path == "index.html";
    ...
    if should_upgrade_default {
        return Ok((
            PortalService::render_digital_avatar_preview_html(&portal).into_bytes(),
            content_type,
        ));
    }

    Ok((inject_avatar_preview_banner(&html).into_bytes(), content_type))
}
