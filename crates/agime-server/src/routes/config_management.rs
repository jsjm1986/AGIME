...
pub async fn get_extensions() -> Result<Json<ExtensionResponse>, StatusCode> {
    let extensions = agime::config::get_all_extensions();
...
pub async fn add_extension(
    ...
    let extensions = agime::config::get_all_extensions();
    let key = agime::config::extensions::name_to_key(&extension_query.name);

    let is_update = extensions.iter().any(|e| e.config.key() == key);

    agime::config::set_extension(ExtensionEntry {
...
pub async fn remove_extension(Path(name): Path<String>) -> Result<Json<String>, StatusCode> {
    let key = agime::config::extensions::name_to_key(&name);
    agime::config::remove_extension(&key);
...
pub async fn init_config() -> Result<Json<String>, StatusCode> {
    ...
    match agime::config::base::load_init_config_from_workspace() {
...
pub async fn upsert_permissions(
    ...
    let mut permission_manager = agime::config::PermissionManager::default();
...
pub async fn create_custom_provider(
    ...
    let config = agime::config::declarative_providers::create_custom_provider(
        &request.engine,
    ...
    if let Err(e) = agime::providers::refresh_custom_providers().await {
...
pub async fn get_custom_provider(
    ...
    let loaded_provider = agime::config::declarative_providers::load_provider(id.as_str())
...
pub async fn remove_custom_provider(Path(id): Path<String>) -> Result<Json<String>, StatusCode> {
    agime::config::declarative_providers::remove_custom_provider(&id)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn update_custom_provider(
    ...
    agime::config::declarative_providers::update_custom_provider(
        &id,
        &request.engine,
        request.display_name,
        request.api_url,
