...
export function ActivityLog({ activities = [], loading = false, maxItems = 5 }: ActivityLogProps) {
  ...
  const getActivityIcon = (type: ActivityItem['type']) => {
    ...
    switch (type) {
      case 'create':
        return (
          <div className={`${iconShellBase} border-[hsl(var(--status-success-text))/0.16] bg-status-success-bg text-status-success-text`}>
      ...
      case 'update':
        return (
          <div className={`${iconShellBase} border-[hsl(var(--status-info-text))/0.16] bg-status-info-bg text-status-info-text`}>
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      case 'delete':
        return (
          <div className={`${iconShellBase} border-[hsl(var(--status-error-text))/0.16] bg-status-error-bg text-status-error-text`}>
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      case 'leave':
        return (
          <div className={`${iconShellBase} border-[hsl(var(--status-warning-text))/0.16] bg-status-warning-bg text-status-warning-text`}>
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      default:
        return (
          <div className="flex h-8 w-8 items-center justify-center rounded-full border border-[hsl(var(--ui-line-soft))/0.78] bg-[hsl(var(--ui-surface-panel-muted))/0.82] text-[hsl(var(--muted-foreground))]">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
