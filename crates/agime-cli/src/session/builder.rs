...
use agime::session::{EnabledExtensionsState, ExtensionState};
...
async fn offer_extension_debugging_help(
    extension_name: &str,
    error_message: &str,
    provider: Arc<dyn agime::providers::base::Provider>,
    interactive: bool,
) -> Result<(), anyhow::Error> {
...
pub async fn build_session(session_config: SessionBuilderConfig) -> CliSession {
    agime::posthog::set_session_context("cli", session_config.resume);
            ...
            .is_some_and(|mc| mc.model_name == model_name)
    {
        ...
        agime::model::ModelConfig::new(&model_name)
            .unwrap_or_else(|e| {
