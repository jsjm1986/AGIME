...
use agime::config::{Config, ExtensionConfig};
use agime_mcp::mcp_server_runner::{serve, McpCommand};
use agime_mcp::{
...
use agime_bench::bench_config::BenchRunConfig;
use agime_bench::runners::bench_runner::BenchRunner;
use agime_bench::runners::eval_runner::EvalRunner;
use agime_bench::runners::metric_aggregator::MetricAggregator;
use agime_bench::runners::model_runner::ModelRunner;
use std::io::Read;
...
enum RecipeCommand {
    ...
    /// Open a recipe in AGIME Desktop
    #[command(about = "Open a recipe in AGIME Desktop")]
    Open {
...
pub struct RecipeInfo {
    ...
    pub sub_recipes: Option<Vec<agime::recipe::SubRecipe>>,
    pub final_output_response: Option<agime::recipe::Response>,
    pub retry_config: Option<agime::agents::types::RetryConfig>,
}
                ...
                (_, _, Some(recipe_name)) => {
                    ...
                    let recipe_version =
                        crate::recipes::search_recipe::load_recipe_file(&recipe_name)
                            .ok()
                            .and_then(|rf| {
                                agime::recipe::template_recipe::parse_recipe_content(
                                    &rf.content,
