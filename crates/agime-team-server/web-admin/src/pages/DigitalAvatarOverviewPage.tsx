      ...
      const governanceResults = await Promise.all(
        avatars.map(async (avatar) => {
          const [queue, governancePayload] = await Promise.all([
            portalApi.listAvatarGovernanceQueue(teamId, avatar.id).catch(() => [] as AvatarGovernanceQueueItemPayload[]),
            portalApi.getAvatarGovernance(teamId, avatar.id).catch(() => null),
          ]);
