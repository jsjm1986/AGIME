...
import { portalApi, type AvatarGovernanceEventPayload, type AvatarInstanceProjection, type PortalSummary } from '../api/portal';
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [teamResult, avatarResult, projectionResult, eventResult, agentResult] = await Promise.all([
        ...
        portalApi.list(teamId, 1, 300, 'avatar'),
        portalApi.listAvatarInstances(teamId).catch(() => [] as AvatarInstanceProjection[]),
        portalApi.listTeamAvatarGovernanceEvents(teamId, 500).catch(() => [] as AvatarGovernanceEventPayload[]),
