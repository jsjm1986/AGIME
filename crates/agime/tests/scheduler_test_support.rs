...
use agime::providers::base::Provider as AgimeProvider;

static TEST_PROVIDER: Lazy<Mutex<Option<Arc<dyn AgimeProvider>>>> = Lazy::new(|| Mutex::new(None));
...
pub async fn set_test_provider(p: Arc<dyn AgimeProvider>) {
    let mut guard = TEST_PROVIDER.lock().await;
    *guard = Some(p);
}

pub async fn get_test_provider() -> Option<Arc<dyn AgimeProvider>> {
