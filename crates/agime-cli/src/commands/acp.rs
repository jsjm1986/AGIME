...
struct GooseAcpSession {
    ...
    tool_requests: HashMap<String, agime::conversation::message::ToolRequest>, // Store tool requests by ID for location extraction
...
fn extract_tool_locations(
    tool_request: &agime::conversation::message::ToolRequest,
    tool_response: &agime::conversation::message::ToolResponse,
...
impl acp::Agent for GooseAcpAgent {
    ...
    async fn prompt(&self, args: acp::PromptRequest) -> Result<acp::PromptResponse, acp::Error> {
        ...
        while let Some(event) = stream.next().await {
            ...
            match event {
                Ok(agime::agents::AgentEvent::Message(message)) => {
                    let mut sessions = self.sessions.lock().await;
                    let session = sessions
                        .get_mut(&session_id)
                        .ok_or_else(acp::Error::invalid_params)?;
