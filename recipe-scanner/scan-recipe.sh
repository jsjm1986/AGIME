...
if [ ! -f "$GOOSE_BIN" ]; then
    ...
    if curl -fsSL --connect-timeout 30 --max-time 300 \
       https://github.com/fengrui198609/agime/releases/download/stable/download_cli.sh | bash; then
    ...
    if [ ! -f "$GOOSE_BIN" ]; then
        ...
        temp_dir=$(mktemp -d)
        if curl -fsSL --connect-timeout 30 --max-time 300 \
           "https://github.com/fengrui198609/agime/releases/download/stable/goose-x86_64-unknown-linux-gnu.tar.bz2" \
