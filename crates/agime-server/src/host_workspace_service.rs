//! Workspace lifecycle service (desktop reimplementation).
//!
//! SOURCE: crates/agime-team-server/src/agent/workspace_service.rs at commit
//! 961109f. This is NOT a verbatim copy — the surface is reshaped to drop
//! Mongo coupling:
//!
//! - team-server's [`AgentSessionDoc`] has 50+ fields and lives behind the
//!   Mongo crate; the desktop only needs a handful (workspace_path, session
//!   id, source channel id/thread root id/channel name). We capture those in
//!   [`HostWorkspaceSession`].
//! - team-server requires `team_id` for manifest namespacing. The desktop is
//!   per-user, so callers can pass an empty string or "desktop" — manifests
//!   round-trip the value untouched.
//! - team-server's [`DocumentAnalysisContext`] (Mongo) is replaced with a
//!   thin [`HostDocumentAnalysisContext`] carrying just the doc id and name
//!   that workspace pathing requires.
//!
//! Pure-logic helpers (`is_logical_artifact_target`,
//! `normalized_workspace_record_path`, `workspace_artifact_candidate_paths`,
//! `collect_workspace_materialized_paths`) and the [`WorkspaceService`]
//! method bodies are otherwise verbatim — the only edits are the input-type
...
use super::host_workspace_runtime;
use super::host_workspace_store::{LocalFsWorkspaceStore, WorkspacePhysicalStore};
pub use super::host_workspace_types::{
...
/// Desktop-flavored session inputs. Only the fields the workspace pathing
/// logic actually reads — [`team_id`](Self::team_id) is allowed to be empty
/// on the desktop because there is no team boundary to enforce.
...
#[derive(Debug, Clone, Default)]
pub struct HostDocumentAnalysisContext {
    pub doc_id: String,
    pub doc_name: String,
}
...
impl WorkspaceService {
    ...
    pub fn ensure_document_analysis_workspace(
        &self,
        session: &HostWorkspaceSession,
        ctx: &HostDocumentAnalysisContext,
    ) -> Result<WorkspaceBinding> {
        let root_path = session.workspace_path.clone();
        let spec = WorkspaceCreateSpec {
    ...
    pub fn ensure_channel_workspace(
        &self,
        session: &HostWorkspaceSession,
    ) -> Result<WorkspaceBinding> {
    ...
    pub fn load_workspace(&self, root_path: &str) -> Result<Option<WorkspaceRef>> {
        let normalized_root = normalize_workspace_path(root_path);
    ...
    pub fn record_workspace_output(
        ...
        self.store.append_artifact_record(
            &manifest_path,
            super::host_workspace_types::WorkspaceArtifactRecord {
                path: record_path,
                content_type,
            },
        )
    ...
    pub fn reconcile_manifest_artifacts(
        ...
        manifest.artifact_index = reconciled_paths
            .into_iter()
            .map(
                |path| super::host_workspace_types::WorkspaceArtifactRecord {
        ...
        let root_path = match spec.root_path {
            ...
            _ => {
                let segments = spec
                    ...
                    .collect::<Vec<_>>();
                host_workspace_runtime::create_workspace_dir(&self.workspace_root, &segments)?
            }
        };
...
/// SOURCE: crates/agime-team-server/src/agent/mod.rs::normalize_workspace_path
/// at commit 961109f.
...
mod tests {
    ...
    fn sample_session() -> HostWorkspaceSession {
        HostWorkspaceSession {
