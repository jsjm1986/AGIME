...
pub fn handle_project_default() -> Result<()> {
    ...
    if projects.is_empty() {
        ...
        let mut command = std::process::Command::new("agime");
        command.arg("session");
        let status = command.status()?;

        if !status.success() {
            println!("Failed to run AGIME. Exit code: {:?}", status.code());
        }
        return Ok(());
    }
    ...
    let _ = intro("AGIME Project Manager");
    ...
    match choice {
        "resume" => {
            ...
            // Build the command to run agime
            let mut command = std::process::Command::new("agime");
            command.arg("session");
