...
import { SkillRegistryWorkspace } from '../components/team/SkillRegistryWorkspace';
    ...
    >
      <AppShell className="team-font-cap">
        <div className="space-y-5">
          <section className="ui-section-panel px-6 py-5">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-2">
                <div className="ui-kicker">{t('teams.tabs.skills')}</div>
                ...
                <p className="max-w-3xl ui-secondary-text text-sm leading-6">
                  {t('teams.resource.skillRegistry.pageDescription', t('teams.resource.skillRegistry.description'))}
                </p>
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                <Button variant="outline" onClick={() => navigate(`/teams/${teamId}?section=toolkit`)}>
                  <ArrowLeft className="mr-2 h-4 w-4" />
                  {t('teams.resource.skillRegistry.backToSkills', '返回技能页')}
                </Button>
                <Button
                  variant="outline"
                  onClick={() => window.open('https://skills.sh', '_blank', 'noopener,noreferrer')}
                >
          ...
          <SkillRegistryWorkspace teamId={teamId} canManage={canManage} />
        </div>
