//! Agent runtime capability resolver.
//!
//! Computes the [`RuntimeCapabilitySnapshot`] that drives prompt composition,
//! tool gating, skill scoping, document policy, and delegation clamping for
//! every chat / portal / channel / scheduled / automation session.
//!
//! Pure data — no Mongo, no broker — so both team and desktop runtimes can
//! call it. Session-side inputs come through [`HostSessionPolicyContext`],
//! which the team server fills from `AgentSessionDoc` and the desktop server
//! fills from its own session record.

use std::collections::HashSet;

use agime_team::models::mongo::{PortalDocumentAccessMode, PortalEffectivePublicConfig};
use agime_team::models::{
    AttachedTeamExtensionRef, BuiltinExtension, DelegationPolicyOverride, SkillBindingMode,
    TeamAgent,
};
...
///
/// Each backend (team / desktop) builds this from its own session
/// representation; the resolver itself never touches a Mongo document.
...
impl AgentRuntimePolicyResolver {
    pub fn resolve(
        agent: &TeamAgent,
        session: Option<&HostSessionPolicyContext>,
...
mod tests {
    ...
    fn sample_session(source: &str) -> HostSessionPolicyContext {
        HostSessionPolicyContext {
            session_source: source.to_string(),
            ..HostSessionPolicyContext::default()
        }
    }
    ...
    fn attached_team_extensions_merge_with_legacy_team_configs_and_obey_overlay_allowlist() {
        ...
        assert!(snapshot
            .extensions
            .effective_allowed_extension_names
            .contains(&"browser_use".to_string()));
        assert!(snapshot
            .extensions
            .effective_allowed_extension_names
            .contains(&"custom_data".to_string()));
        assert!(!snapshot
            .extensions
            .effective_allowed_extension_names
