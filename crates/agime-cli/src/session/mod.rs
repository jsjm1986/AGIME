...
use agime::conversation::message::{ActionRequiredData, Message, MessageContent};
...
impl CliSession {
    ...
    pub async fn add_extension(&mut self, extension_command: String) -> Result<()> {
        ...
        let config = ExtensionConfig::Stdio {
            ...
            envs: Envs::new(envs),
            env_keys: Vec::new(),
            description: agime::config::DEFAULT_EXTENSION_DESCRIPTION.to_string(),
                ...
                result = stream.next() => {
                    match result {
                        ...
                        Some(Err(e)) => {
                            ...
                            if e.downcast_ref::<agime::providers::errors::ProviderError>()
                                .map(|provider_error| matches!(provider_error, agime::providers::errors::ProviderError::ContextLengthExceeded(_)))
                                .unwrap_or(false) {
    ...
    pub async fn get_session(&self) -> Result<agime::session::Session> {
