...
import { portalApi, type AvatarInstanceProjection, type PortalSummary } from '../api/portal';
...
export default function DigitalAvatarPolicyCenterPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const avatarProjections = await portalApi.listAvatarInstances(teamId).catch(() => []);
  ...
  const handleApplyToExisting = useCallback(async () => {
    ...
    try {
      for (const avatar of scopedAvatars) {
        try {
          const detail = await portalApi.get(teamId, avatar.id);
          const nextConfig: Record<string, unknown> = { ...policy };
          await portalApi.updateAvatarGovernance(teamId, avatar.id, { config: nextConfig });
          await portalApi.update(teamId, avatar.id, {
