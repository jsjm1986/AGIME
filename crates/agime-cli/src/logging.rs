...
use agime::tracing::{langfuse_layer, otlp_layer};
use agime_bench::bench_session::BenchAgentError;
use agime_bench::error_capture::ErrorCaptureLayer;
...
fn setup_logging_internal(
    ...
    let mut setup = || {
        result = (|| {
            let log_dir = agime::logging::prepare_log_directory("cli", true)?;
            let timestamp = chrono::Local::now().format("%Y%m%d_%H%M%S").to_string();
            let log_filename = if let Some(name) = name {
                format!("{}-{}.log", timestamp, name)
            } else {
            ...
            let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
                ...
                EnvFilter::new("")
                    ...
                    // Set agime module to DEBUG
                    .add_directive("agime=debug".parse().unwrap())
                    // Set agime-cli to INFO
                    .add_directive("agime_cli=info".parse().unwrap())
...
mod tests {
    ...
    fn test_log_directory_creation() {
        ...
        let log_dir = agime::logging::prepare_log_directory("cli", true).unwrap();
        assert!(log_dir.exists());
        assert!(log_dir.is_dir());

        // Verify directory structure
        let path_components: Vec<_> = log_dir.components().collect();
        assert!(path_components.iter().any(|c| c.as_os_str() == "agime"));
